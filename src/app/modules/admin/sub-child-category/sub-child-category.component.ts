...
export class SubChildCategoryComponent implements OnInit {
  ...
  constructor(
    ...
    private subChildCategoryService: SubChildCategoryService,
  ...
  getData() {
    this.subChildCategoryService
      .getSubChildCategories({ params: this.tablePaging })
      .subscribe((res: any) => {
        this.loading = false;
  ...
  getNextData() {
    ...
    this.userDataPromise = this.subChildCategoryService
      .getSubChildCategories({ params: this.tablePaging })
  ...
  applyCategoryFilter(filterValue: string) {
    ...
    //this.getNextData();
    this.userDataPromise = this.subChildCategoryService
      .filterSubChildCategories(filterData)
      .subscribe((res: any) => {
        this.loading = false;
        this.subChildCategories = res.SubChildCategories;
        this.subChildCategories.length = res.total;
  ...
  postFormInput() {
    ...
    if (this.subChildCategoriesForm.invalid) {
      console.log(
        "this.subChildCategoriesForm",
        this.subChildCategoriesForm.value
      );
      return false;
    }
    if (this.seletedSubCategory) {
      ...
      if (this.showPreview == false) {
        ...
        this.subChildCategoryService
          .addSubChildCategories(this.subChildCategoriesForm.value)
          .subscribe(
            (res: any) => {
              this.modalService.dismissAll();
              this.subChildCategoriesForm.reset();
  ...
  postData() {
    ...
    if (this.showPreview == true) {
      ...
      if (file.match(/png/g) || file.match(/jpeg/g) || file.match(/jpg/g)) {
        this.subChildCategoryService
          .uploadSubChildCategoryBanner(formData)
          .subscribe(
            (res: any) => {
              this.subChildCategoriesForm.patchValue({
  ...
  deleteCategory(deleteCategory: any) {
    if (confirm("Are you sure to delete ?")) {
      this.subChildCategoryService
        .deleteSubChildCategories(deleteCategory)
        .subscribe((res: any) => {
          this.getNextData();
        });
    }
  ...
  BulkDelete() {
    if (
      ...
      if (confirm("Are you sure to delete ?")) {
        this.subChildCategoryService
          .bulkDelete(this.setBulkDeleteItems)
          .subscribe(
            (res: any) => {
